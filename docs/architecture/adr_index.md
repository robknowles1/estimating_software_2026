...
|-----|-------|--------|------|-------------|
| [ADR-015](ADR-015-presets-autopopulation.md) | Product Presets Auto-Population — Two-Level Indirection and Slot Code Model | proposed | 2026-05-27 | Ten `<slot>_slot_code` string columns on `products`; new `ProductSlotResolver` service resolves codes against estimate price book at line item creation; PR 41 / SPEC-022 merges as-is; Presets feature is SPEC-029. |
| [ADR-012](ADR-012-digital-ocean-hosting.md) | Digital Ocean Hosting with Kamal 2 | accepted | 2026-04-25 | Kamal 2 on DO Droplets; ghcr.io registry (free); staging on a dedicated 1 vCPU Droplet (64.23.238.38) on ports 3000/3001; `deploy_staging` CI job fires on every merge to main. |
