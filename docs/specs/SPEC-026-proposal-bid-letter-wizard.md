...
A contact is **required** to advance past the opening step (the existing `validating_opening` contact-presence validation is retained). The opening step shows a prominent **"+ Add contact"** button next to the contact dropdown **at all times** — whether or not the client already has contacts. When the client has zero contacts, the empty (`include_blank`, required) select is still rendered alongside a "no contacts yet" hint and the button.

The "+ Add contact" button opens an **inline modal dialog** (Hotwire: a Stimulus modal controller + Turbo Frame/Stream) that keeps the estimator on the opening step — replacing the previous dead-end round-trip to the standalone contacts page, whose controller redirected back to the client page after saving. The modal is a quick-add form with four fields: `first_name` (required), `last_name` (required), `email`, `phone` (title/notes remain on the full contacts page). Submitting the modal creates the contact via Turbo Stream (no full navigation):

- **Success:** the contact dropdown is rebuilt (via a `replace` of the `proposal_contact_field` Turbo target) with the new contact **auto-selected**, and the modal closes (the dialog is re-rendered closed/empty so it can be reopened). HTTP 200 turbo_stream.
- **Failure** (e.g. missing first/last name): the modal stays open and re-renders its inner Turbo Frame (`add_contact_modal_frame`) with field errors. HTTP 422 turbo_stream; no navigation; no contact created.

The modal posts to the existing `ContactsController#create` (`POST /clients/:id/contacts`) with a hidden `estimate_id` so the success stream knows which wizard select to rebuild (`estimate_id` is read from `params`, never mass-assigned to the contact; the estimate's client is verified to match before exposure). The standalone HTML create flow (redirect to the client page) is unchanged. Accessibility: the dialog uses `role="dialog"` + `aria-modal="true"` + `aria-labelledby`; focus moves to the first field on open, is trapped while open, and returns to the "+ Add contact" button on close; Cancel, the × button, Escape, and a backdrop click all close the modal.
...
E1: The estimate has no contacts on the client — the opening step still renders the (empty, `include_blank`, required) contact select plus a "no contacts yet" hint and a prominent "+ Add contact" button. The estimator adds a contact inline via the add-contact modal (see R5) without leaving the wizard; the new contact is auto-selected and they can proceed. Contact remains required to advance.
...
4. Detects alternate line items using `ALTERNATE_PREFIXES` and creates `ProposalAlternate` records, setting `display_cost` on a gross/burdened basis (OQ-E resolved): `line_item_results[line_item.id][:non_burdened_total] * burden_multiplier`, rounded to 2 decimals. This is consistent with `total_amount` using `burdened_total`. Job-level fixed costs are not attributable to a single alternate line and are therefore excluded. The line item's entry is **fetched** from `line_item_results` (raising `KeyError`) rather than defaulting to 0: alternates are drawn from `estimate.line_items` and `line_item_results` is keyed by those same line items, so a missing entry is an internal inconsistency that fails fast and rolls back the transaction instead of silently persisting a $0 alternate on a client-facing bid.
5. Queries `estimate.line_items` and collects distinct room name strings, stripping surrounding whitespace and skipping blank/whitespace-only rooms (i.e. beyond the `where.not(room: [nil, ""])` note, names like `"   "` are dropped and `" Kitchen "` is stored as `"Kitchen"` and deduped with `"Kitchen"`). Stripping/dedupe is done Ruby-side (`pluck(:room).filter_map { |r| r&.strip.presence }.uniq`) since the per-estimate line-item set is small. SPEC-025 stores room as a plain string column on `line_items`; there is no `rooms` table. Blank/whitespace-only rooms are skipped because `room_name` is presence-validated and would otherwise create an invalid `ProposalInclusion`. For each unique stripped room name, creates one `ProposalInclusion` with `room_name` set to the string and `bullet_points` blank. If no line items have a non-blank `room`, the inclusions list starts empty.
...
| OQ | Question | Status |
|----|----------|--------|
| OQ-A | Does the PDF need a company letterhead image (logo)? If so, how will it be stored (config file, database, ActiveStorage)? | **Resolved (provisional).** Text-only letterhead for v1 (PR 4); an image logo remains a deferred follow-up. |
| OQ-B | What is the exact company address block to embed in the PDF header? | **Resolved (provisional).** PR 4 implements `Company.address` (in `app/models/company.rb`), read from `Rails.application.credentials.dig(:company, :address)`, then `ENV["COMPANY_ADDRESS"]`, falling back to a default extracted from Blake's template (`TrimArt / 601 Boro Street / Kaysville, UT 84037`). Pending Blake's final confirmation of the exact name/address block (`# TODO(OQ-B)` left in the model). |
