...
4. Detects alternate line items using `ALTERNATE_PREFIXES` and creates `ProposalAlternate` records, setting `display_cost` from the alternate line item's contribution to `EstimateTotalsCalculator` (burdened total pro-rated or line-item non-burdened subtotal — confirm with Blake which cost basis applies; see OQ-E).
5. Queries `estimate.line_items.where.not(room: nil).pluck(:room).uniq` to get distinct room name strings (SPEC-025 stores room as a plain string column on `line_items`; there is no `rooms` table). For each unique room name, creates one `ProposalInclusion` with `room_name` set to the string and `bullet_points` blank. If no line items have a non-nil `room`, the inclusions list starts empty.
...
| OQ-C | Does SPEC-025 (room tracking from CSV import) exist and what is the room model structure? | **Resolved.** SPEC-025 is not yet written but the confirmed data model is: `room` is a plain string column on `line_items` (no separate `rooms` table). `BuildService` seeds inclusions via `estimate.line_items.where.not(room: nil).pluck(:room).uniq`. R8 and `BuildService` step 5 have been updated accordingly. |
| OQ-D | Should `payment_terms` (residential mode) have a default value pre-populated? | **Open.** Confirm with Blake. If yes, add to `BuildService` seed logic, similar to clarifications/exclusions. |
| OQ-E | Should alternate costs shown in the proposal be net (before markup) or gross (after profit/overhead)? | **Open.** The spec currently uses the line item's calculated cost as stored in the estimate. Confirm with Blake whether the alternate total should include or exclude overhead/profit. |
...
- SPEC-025 (Room Tracking / CSV Import Rooms) — **not yet written.** Room data is a plain string column `room` on `line_items` (confirmed). `BuildService` seeds inclusions from `estimate.line_items.where.not(room: nil).pluck(:room).uniq`. SPEC-025 must add the `room` column to `line_items` before the inclusions seeding step will produce any rows. The feature can be developed in parallel: if the column does not yet exist, `BuildService` will produce an empty inclusions list (the `where.not(room: nil)` query will return zero rows), which is a safe degraded state.
